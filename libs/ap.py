...
def startAP():
    ...
    d.bmp("/data/picture/Step1.bmp",0,0) #步骤1，连接AP热点。
    d.show()
    ...
    d.bmp("/data/picture/Step2.bmp", 0, 0) #步骤2，登录192.168.4.1进行配网。
    d.show()
                    ...
                    if ret != None: #获取信息成功
                        ...
                        if city_judge(ret[2]): #有该城市
                            ...
                            if ret_ip is not None:
                                ...
                                response = config_page()
                                send_response(conn, response)
                                d.bmp("/data/picture/error2.bmp",0,0) #步骤2，登录192.168.4.1进行配网。
                                d.show()
                            ...
                            response = config_page()
                            send_response(conn, response)
                            d.bmp("/data/picture/error1.bmp",0,0) #步骤2，登录192.168.4.1进行配网。
                            d.show()
                        ...
                        response = config_page()
                        send_response(conn, response)
                        d.bmp("/data/picture/error3.bmp",0,0) #步骤2，登录192.168.4.1进行配网。
                        d.show()
